...
# roughly equivalent function in bash
# cat ~/.config/fastfetch/distros.json | jq ".$(grep -oP '(?<=^NAME=")[^"]+(?=")' /etc/os-release | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z]//g; s/linux$//g; s/gnu$//g; s/os$//g')[]" -r | env fastfetch --file /dev/stdin  $argv
